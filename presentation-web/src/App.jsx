...
function PreprocessingResultSlide() {
  ...
  return <section className="preprocessing-result-slide" aria-labelledby="preprocessing-result-title">
    ...
    <div className="preprocess-split">
      <article className="cleaning-panel"><p className="panel-label">01 · PEMBERSIHAN BARIS DATA</p><div className="cleaning-table"><div className="cleaning-head"><span>TAHAP</span><span>DIHAPUS</span><span>SISA BARIS</span></div>{cleaningRows.map(([stage, description, removed, remaining]) => <div className="cleaning-row" key={stage}><strong>{stage}<small>{description}</small></strong><b>{removed}</b><em>{remaining}</em></div>)}</div><aside className="key-clean-data"><span>SETELAH FILTER ITEM NON-PRODUK</span><b>528.515</b><p>baris produk relevan untuk dianalisis</p></aside><p className="panel-total"><b>517.721</b> baris data bersih setelah deduplikasi</p></article>
