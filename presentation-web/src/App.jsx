...
function SupportComparisonSlide() {
  ...
  return <section className="support-comparison-slide" aria-labelledby="support-comparison-title"><div className="section-heading"><p>15 — ANALISIS PERBANDINGAN</p><h2 id="support-comparison-title">Dampak Perubahan <span>Support</span></h2></div><p className="support-comparison-intro">Perbandingan dampak perubahan minimum support terhadap hasil PCY.</p><div className="support-decision-grid">{options.map(([support, threshold, result, tradeoff, use]) => <article className={support === '3%' ? 'selected-support' : ''} key={support}><b>{support}</b><span>ambang {threshold}</span><strong>{result}</strong><p>{tradeoff}</p><em>{use}</em></article>)}</div><aside className="support-comparison-note"><b>Kesimpulan perbandingan:</b> 1% cocok untuk eksplorasi pola yang lebih luas, 2% menjadi alternatif moderat, sedangkan <strong>3% dipilih</strong> karena memberi aturan yang tetap bermakna dengan beban komputasi jauh lebih efisien.</aside></section>
}
function SupportRulesAppendixSlide() {
  const datasets = [
    { support: '1%', subtitle: 'Ambang 182 · 1.107 frequent pair', rules: [['HERB MARKER THYME → HERB MARKER ROSEMARY', '1,2% · 93,64% · 0,923'], ['HERB MARKER CHIVES → HERB MARKER PARSLEY', '1,1% · 92,34% · 0,910'], ['HERB MARKER ROSEMARY → HERB MARKER THYME', '1,2% · 92,08% · 0,908']] },
    { support: '2%', subtitle: 'Ambang 365 · 109 frequent pair', rules: [['PINK REGENCY TEACUP AND SAUCER → GREEN REGENCY TEACUP AND SAUCER', '3,5% · 83,18% · 0,776'], ['WOODEN STAR CHRISTMAS SCANDINAVIAN → ROSES REGENCY TEACUP AND SAUCER', '4,2% · 75,93% · 0,733'], ['PINK REGENCY TEACUP AND SAUCER → ROSES REGENCY TEACUP AND SAUCER', '3,3% · 78,71% · 0,729']] },
    { support: '3%', subtitle: 'Ambang 548 · 15 frequent pair', rules: [['PINK REGENCY TEACUP AND SAUCER → GREEN REGENCY TEACUP AND SAUCER', '3,5% · 83,18% · 0,776'], ['PINK REGENCY TEACUP AND SAUCER → ROSES REGENCY TEACUP AND SAUCER', '3,3% · 78,71% · 0,729'], ['GREEN REGENCY TEACUP AND SAUCER → ROSES REGENCY TEACUP AND SAUCER', '4,2% · 75,67% · 0,699']] },
  ]
