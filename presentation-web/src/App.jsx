...
const challenges = [
  ...
  <>Sebagian besar pasangan sebenarnya tidak pernah mencapai <em>batas minimum support</em>.</>,
]
const solutions = [
...
function AssociationRulesSlide() {
  return <section className="association-slide" aria-labelledby="association-title"><div className="section-heading"><p>11 — ASSOCIATION RULE MINING · SUPPORT 3%</p><h2 id="association-title">Association Rule Mining <span>— Top 5 Rules</span></h2></div><div className="rules-table" role="table" aria-label="Top 5 Association Rules berdasarkan interest pada minimum support 3 persen"><div className="rules-head" role="row"><span>PERINGKAT</span><span>ASSOCIATION RULE</span><span><InfoTerm detail="Proporsi seluruh basket yang memuat kedua produk dalam rule.">SUPPORT</InfoTerm></span><span><InfoTerm detail="Peluang produk tujuan ikut dibeli ketika produk awal dibeli: count(A dan B) ÷ count(A).">CONFIDENCE</InfoTerm></span><span><InfoTerm detail="Tambahan peluang membeli produk tujuan setelah produk awal dibeli: confidence(A→B) − support(B). Hanya rule positif > 0,1 yang ditampilkan.">INTEREST</InfoTerm></span></div>{associationRules.map(([rule, support, confidence, interest], index) => <div className={`rules-row ${index === 0 ? 'top-rule' : 'supporting-rule'}`} role="row" key={rule}><span>0{index + 1}</span><strong>{rule}{index === 0 && <small className="rule-badge">RULE TERBAIK</small>}</strong><b>{support}</b><b>{confidence}</b><em>{interest}</em></div>)}</div><div className="association-bottom"><aside className="rule-explanation"><p><strong>Interpretasi:</strong> Pada minimum support <b>3%</b>, rule <b>PINK REGENCY TEACUP AND SAUCER → GREEN REGENCY TEACUP AND SAUCER</b> memiliki interest tertinggi, yaitu <b>0,776</b>. Artinya, kedua produk memiliki keterkaitan pembelian yang kuat.</p></aside><div className="metric-explanation"><p><b>Confidence (A → B)</b> = support(A, B) ÷ support(A).</p><p><b>Support</b> menunjukkan proporsi basket yang memuat dua produk bersama.</p><p><b>Interest</b> = confidence(A → B) − support(B); nilai positif berarti A meningkatkan peluang B dibanding popularitas B secara umum.</p></div></div><p className="association-conclusion">Dari <strong>15 frequent pair</strong> pada support 3%, seri produk <strong>Regency Teacup and Saucer</strong> muncul sebagai peluang bundling dan rekomendasi silang.</p></section>
}
function ProductInsightSlide() {
  return <section className="product-insight-slide" aria-labelledby="insight-title"><div className="section-heading"><p>12 — INSIGHT PRODUK</p><h2 id="insight-title">Pola Pembelian <span>Regency Teacup</span></h2></div><div className="insight-grid"><article className="insight-highlight"><span>INTEREST TERTINGGI</span><strong>0,776</strong><p>PINK REGENCY TEACUP AND SAUCER<br />→ GREEN REGENCY TEACUP AND SAUCER</p></article><article className="insight-copy"><h3>Temuan Produk</h3><ul><li><strong>PINK REGENCY TEACUP AND SAUCER, GREEN REGENCY TEACUP AND SAUCER, dan ROSES REGENCY TEACUP AND SAUCER</strong> sering muncul dalam transaksi yang sama.</li><li>Pembeli <strong>Green Regency Teacup and Saucer</strong> juga cenderung membeli <strong>ROSES REGENCY TEACUP AND SAUCER</strong>, dan sebaliknya.</li><li>Dari setiap 100 pembeli <strong>Pink Regency Teacup and Saucer</strong>, sekitar <strong>83 pembeli</strong> juga membeli <strong>GREEN REGENCY TEACUP AND SAUCER</strong>.</li></ul></article></div></section>
}
const conclusions = [
...
function SupportComparisonSlide() {
  ...
  return <section className="support-comparison-slide" aria-labelledby="support-comparison-title"><div className="section-heading"><p>15 — ANALISIS PERBANDINGAN</p><h2 id="support-comparison-title">Dampak Perubahan <span>Support</span></h2></div><p className="support-comparison-intro">Perbandingan dampak perubahan minimum support terhadap hasil PCY.</p><div className="support-decision-grid">{options.map(([support, threshold, result, tradeoff, use]) => <article className={support === '3%' ? 'selected-support' : ''} key={support}><b>{support}</b><span>batas minimum {threshold}</span><strong>{result}</strong><p>{tradeoff}</p><em>{use}</em></article>)}</div><aside className="support-comparison-note"><b>Kesimpulan perbandingan:</b> 1% cocok untuk eksplorasi pola yang lebih luas, 2% menjadi alternatif moderat, sedangkan <strong>3% dipilih</strong> karena memberi aturan yang tetap bermakna dengan beban komputasi jauh lebih efisien.</aside></section>
}
function SupportRulesAppendixSlide() {
  const datasets = [
    { support: '1%', subtitle: 'Batas minimum 182 · 1.107 frequent pair', rules: [['HERB MARKER THYME → HERB MARKER ROSEMARY', '1,2% · 93,64% · 0,923'], ['HERB MARKER CHIVES → HERB MARKER PARSLEY', '1,1% · 92,34% · 0,910'], ['HERB MARKER ROSEMARY → HERB MARKER THYME', '1,2% · 92,08% · 0,908']] },
    { support: '2%', subtitle: 'Batas minimum 365 · 109 frequent pair', rules: [['PINK REGENCY TEACUP AND SAUCER → GREEN REGENCY TEACUP AND SAUCER', '3,5% · 83,18% · 0,776'], ['WOODEN STAR CHRISTMAS SCANDINAVIAN → ROSES REGENCY TEACUP AND SAUCER', '4,2% · 75,93% · 0,733'], ['PINK REGENCY TEACUP AND SAUCER → ROSES REGENCY TEACUP AND SAUCER', '3,3% · 78,71% · 0,729']] },
    { support: '3%', subtitle: 'Batas minimum 548 · 15 frequent pair', rules: [['PINK REGENCY TEACUP AND SAUCER → GREEN REGENCY TEACUP AND SAUCER', '3,5% · 83,18% · 0,776'], ['PINK REGENCY TEACUP AND SAUCER → ROSES REGENCY TEACUP AND SAUCER', '3,3% · 78,71% · 0,729'], ['GREEN REGENCY TEACUP AND SAUCER → ROSES REGENCY TEACUP AND SAUCER', '4,2% · 75,67% · 0,699']] },
  ]
